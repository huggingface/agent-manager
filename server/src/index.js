...
import {
  PORT, BIND_HOST, INTERNAL_HOST, PUBLIC_DIR, DATA_DIR, WORKSPACES_DIR, SKILLS_DIR,
...
\`\`\`sh
curl -s "http://\${AM_HOST:-${INTERNAL_HOST}}:\${AM_PORT:-${PORT}}/api/operations?limit=100" | jq .operations
\`\`\`
...
\`\`\`sh
curl -sS --fail -X POST "http://\${AM_HOST:-${INTERNAL_HOST}}:\${AM_PORT:-${PORT}}/api/crons?from=$AM_ID" \\
...
curl -s "http://\${AM_HOST:-${INTERNAL_HOST}}:\${AM_PORT:-${PORT}}/api/crons" | jq .crons
\`\`\`
...
\`\`\`sh
curl -s "http://\${AM_HOST:-${INTERNAL_HOST}}:\${AM_PORT:-${PORT}}/api/usage?provider=claude" | jq .providers.claude
curl -s "http://\${AM_HOST:-${INTERNAL_HOST}}:\${AM_PORT:-${PORT}}/api/usage?provider=codex" | jq .providers.codex
curl -s "http://\${AM_HOST:-${INTERNAL_HOST}}:\${AM_PORT:-${PORT}}/api/usage?provider=opencode" | jq .providers.opencode
curl -s "http://\${AM_HOST:-${INTERNAL_HOST}}:\${AM_PORT:-${PORT}}/api/usage?provider=hermes" | jq .providers.hermes
curl -s "http://\${AM_HOST:-${INTERNAL_HOST}}:\${AM_PORT:-${PORT}}/api/usage?provider=openclaw" | jq .providers.openclaw
\`\`\`
...
### See who is here
\`\`\`sh
curl -s "http://\${AM_HOST:-${INTERNAL_HOST}}:\${AM_PORT:-${PORT}}/api/agents?from=$AM_ID" | jq .
\`\`\`
...
\`\`\`sh
curl -s "http://\${AM_HOST:-${INTERNAL_HOST}}:\${AM_PORT:-${PORT}}/api/agents/$ID/tail?lines=120" | jq -r .text
curl -s "http://\${AM_HOST:-${INTERNAL_HOST}}:\${AM_PORT:-${PORT}}/api/agents/$ID/wait?timeout=300&settle=15&from=$AM_ID"
\`\`\`
...
\`\`\`sh
( until curl -s "http://\${AM_HOST:-${INTERNAL_HOST}}:\${AM_PORT:-${PORT}}/api/agents/$ID/wait?timeout=300&settle=15&from=$AM_ID" \\
        > /tmp/wait-$ID.json \\
...
\`\`\`sh
curl -s -X POST "http://\${AM_HOST:-${INTERNAL_HOST}}:\${AM_PORT:-${PORT}}/api/agents/$ID/prompt?from=$AM_ID" \\
...
### Launch a new agent
\`\`\`sh
curl -s -X POST "http://\${AM_HOST:-${INTERNAL_HOST}}:\${AM_PORT:-${PORT}}/api/agents?cli=claude&name=reviewer&from=$AM_ID" \\
  -H 'content-type: text/plain' --data-binary @- <<'EOF'
...
\`\`\`sh
GID=$(curl -sS --fail -X POST "http://\${AM_HOST:-${INTERNAL_HOST}}:\${AM_PORT:-${PORT}}/api/groups?from=$AM_ID" \\
  -H 'content-type: application/json' -d '{"name":"taskforce"}' | jq -r .id)

curl -sS --fail -X POST "http://\${AM_HOST:-${INTERNAL_HOST}}:\${AM_PORT:-${PORT}}/api/agents?cli=claude&group=$GID&from=$AM_ID" \\
...
### Stop an agent
\`\`\`sh
curl -s -X POST "http://\${AM_HOST:-${INTERNAL_HOST}}:\${AM_PORT:-${PORT}}/api/agents/$ID/stop?from=$AM_ID"
\`\`\`
...
\`\`\`sh
curl -s -X POST "http://\${AM_HOST:-${INTERNAL_HOST}}:\${AM_PORT:-${PORT}}/api/notify?from=$AM_ID" \\
...
\`\`\`sh
(sleep 600 && curl -s -X POST "http://\${AM_HOST:-${INTERNAL_HOST}}:\${AM_PORT:-${PORT}}/api/notify?from=$AM_ID" \\
  -H 'content-type: application/json' \\
...
server.listen(PORT, BIND_HOST, () => {
  ...
  crons.startScheduler(async (id, trigger) => {
    // INTERNAL_HOST, not a hardcoded loopback: with BIND_HOST set to a specific
    // interface the server never binds 127.0.0.1, and every scheduled fire
    // would be refused before it reached the route.
